...
def join_dataframes(gdp_data,pop_data,fos_data):
    ...
    fos_data.rename(columns={'Country':'Country Name'}, inplace=True)
    joined_df = pd.merge(gdp_melt, pop_melt,how='left', left_on=['Country Name','Year'], right_on=['Country Name','Year'])
    joined_df = pd.merge(joined_df, fos_data, how='inner', left_on=['Country Name','Year'], right_on=['Country Name','Year'])
    return joined_df
